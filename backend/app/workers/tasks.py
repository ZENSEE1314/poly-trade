    ...
    if use_real:
        try:
            ...
            if user.wallet.mode == "private_key":
                ...
                # API-key path: see services/polymarket.py to extend (omitted here).
                log.warning("api_key mode live trading not implemented in this snippet")
...
def reconcile_open_trades() -> dict:
    ...
    try:
        ...
        for t in open_trades:
            ...
            try:
                open_idx = next(i for i, v in enumerate(df_ts) if v >= t.window_ts)
                close_idx = next(i for i, v in enumerate(df_ts) if v >= close_ts) - 1
