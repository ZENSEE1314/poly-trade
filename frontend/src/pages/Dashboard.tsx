...
export default function Dashboard() {
  ...
  return (
    <div>
      <h2 style={{ marginTop: 0 }}>Live Prediction</h2>
      {latest ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
          <Tile label="BTC spot" value={`$${latest.btc_price.toFixed(2)}`} />
        ...
        <div style={{ height: 220 }}>
          <ResponsiveContainer>
            <LineChart data={preds}>
              <XAxis dataKey="window_ts" tickFormatter={t => new Date(t * 1000).toLocaleTimeString().slice(0,5)} stroke="#6b7785" />
      ...
      {stats && (
        <div style={card(18)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
            ...
            <Tile label="Win rate"
                  value={stats.win_rate == null ? "—" : pct(stats.win_rate)} />
            <Tile label="PnL (USDC)"
                  value={stats.pnl_usdc_7d?.toFixed(2)}
